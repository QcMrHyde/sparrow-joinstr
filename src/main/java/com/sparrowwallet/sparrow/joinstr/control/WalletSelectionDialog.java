...
public class WalletSelectionDialog extends Dialog<Void> {
    ...
    public WalletSelectionDialog(Map<Wallet, Storage> wallets) {
        ...
        VBox vbox = new VBox();
        vbox.getChildren().addAll(lvWallets, errorLabel);
        ...
        dialogPane.getButtonTypes().addAll(registerButtonType);

        setOnCloseRequest(this::doCloseRequest);
    ...
    private void doCloseRequest(Event e) {
        ...
        if (selectedWallet == null) {
            errorLabel.setText("No wallet selected");
            e.consume();
        }
